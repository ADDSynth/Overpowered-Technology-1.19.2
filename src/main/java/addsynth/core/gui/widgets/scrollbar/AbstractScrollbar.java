...
public abstract class AbstractScrollbar<E, L extends AbstractListEntry<E>> extends AbstractWidget {
  ...
  private final void recalculateScrollbar(){
    try{
      ...
      scrollbar_system.calculatePositions(y, scrollbar.getHeight(), height, visible_elements, list_length);
  ...
  public void render(PoseStack matrix, int p_render_1_, int p_render_2_, float p_render_3_){
    ...
    // Background
    scrollbar_background_texture.drawVerticalSplit(matrix, x, y, scrollbar_gui_width, height);
  ...
  protected final void onDrag(double mouse_x, double mouse_y, double screen_x, double screen_y){
    scrollbar.setY(Mth.clamp((int)mouse_y - mouse_distance, y, y + height - scrollbar.getHeight()));
